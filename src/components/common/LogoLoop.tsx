...
const useResizeObserver = (callback: () => void, elements: Array<React.RefObject<HTMLElement | null>>, dependencies: any[]) => {
  useEffect(() => {
...
const useAnimationLoop = (
  ...
  useEffect(() => {
    ...
    if (seqSize > 0) {
      ...
      (track as HTMLDivElement).style.transform = transformValue;
    }
...
export type LogoItem = any; // Simplified typing to ensure compatibility
...
export const LogoLoop = memo(function LogoLoop({
  ...
  const [copyCount, setCopyCount] = useState<number>(ANIMATION_CONFIG.MIN_COPIES as number);
  ...
  const updateDimensions = useCallback(() => {
    ...
    if (isVertical) {
      ...
      if (sequenceHeight > 0) {
        ...
        const viewport = (containerRef.current?.clientHeight ?? parentHeight ?? sequenceHeight) as number;
  ...
  useAnimationLoop(trackRef, targetVelocity, seqWidth, seqHeight, isHovered, effectiveHoverSpeed as any, isVertical);
  ...
  const renderLogoItem = useCallback((item: any, key: React.Key) => {
    if (renderItem) {
      return (
...
(LogoLoop as any).displayName = 'LogoLoop';
